...
from Bot_recording.record_bot.json_database.write_read_json import *
...
async def add_members_vvpd(message: types.Message, state: FSMContext):
    try:
        ...
        if message_member[0] in get_dict_members.keys() or message_member[1] in get_dict_members.values():
            await message.answer("Извините, но вы уже записаны, если вы хотите выписаться то обратитесь к админу "
                                 "ONI-CHAN")
            await state.set_state(AddPracticeVVPD.add_to_dict)
        else:
            get_dict_members[message_member[0]] = message_member[1]
            get_dict_members = dict(sorted({int(k): v for k, v in get_dict_members.items()}.items()))
